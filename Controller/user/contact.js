...
const contactsub = require("../../Models/contactUserQuestion");
...
const contactTopic = require("../../Models/contactUserTopic");
...
const contactUsissue = async (req, res) => {
  const aboutUsData = {
    ...req.body,
    createdOn: new Date(),
  };
  new ContactUsIssue(aboutUsData)
    .save()
    .then((data) => {
      return res.status(ResponseCode.errorCode.success).json({
        status: true,
        message: "Contact issue send successfully",
      });
    })
    .catch((error) => {
      const errors = DBerror(error);
